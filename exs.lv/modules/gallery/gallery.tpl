...
{imgblock-seperator}<a class="{image-list-sel}" href="/gallery/{user-id}/{image-list-id}#images"><img src="http://img.exs.lv/{image-list-thb}" alt="{image-list-linkid}" /><span>{image-list-posts}</span></a>
				<!-- START BLOCK : image-list-node-->
			</div>
		</div>
	</div>
	...
	<div id="gal-large" style="width:{width}px;height:{height}px" class="prew-next">
		...
		<img width="{width}" height="{height}" src="http://img.exs.lv/{image-url}" alt="attēls" title="{image-title}" />
		{newer}
		{older}

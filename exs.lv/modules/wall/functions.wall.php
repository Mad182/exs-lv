...
function get_index_events() {
	...
	if ($actions) {
		...
		foreach ($actions as $action) {
			...
			$out .= '<li><img class="av" width="45" height="45" src="' . $action->avatar . '" alt="" /><div class="event-content"><span class="post-time">' . time_ago($action->time) . ', ' . $user->nick . '</span><br />' . $action->action . '</div><div class="c"></div></li>';
		}
		$out .= '</ul>';
	}
	return $out;

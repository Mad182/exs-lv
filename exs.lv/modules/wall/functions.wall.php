...
function strip_selected_tags($text, $tags = []) {
	...
	$text = str_ireplace('src="http://', 'src="https://', $text);
...
function get_index_events() {
	...
	$actions = $db->get_results("SELECT `user`, `action`, `avatar`, `time` FROM `userlogs` WHERE `lang` = '$lang' ".$private." ORDER BY `time` DESC LIMIT 8");

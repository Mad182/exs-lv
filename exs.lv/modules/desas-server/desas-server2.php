...
if (!$game = $db->get_row("SELECT * FROM desas WHERE (user_1 = '$auth->id' OR user_2 = '$auth->id') AND ((status = 0 OR status = 1) OR loser_seen = 0)")) {
	...
	if (isset($_GET['var1']) && $_GET['var1'] == 'drop') {
		...
		if($game->modified >= date('Y-m-d H:i:s', time()-8) || $out['me'] == $game->waiting_for) {
			//pr('you loose');
			$out['overlay'] = 'Tu zaudēji :( <small>Spēli var pamest tikai ja pretinieks ir neaktīvs 10 sekundes</small>';
			$out['finished'] = 1;
			$out['winner'] = $winner;
			$out['alert'] = '';
			$winner = 0;
			...
			$db->query("UPDATE `desas` SET `winner` = '$winner', `waiting_for` = '0', status = '2', loser_seen = 1, modified = NOW() WHERE id = '$game->id'");

			$out['fields'] = unserialize($game->data);
			$lost = 1;
		} else {

...
{
	include ROOT . '/pages/league/NHL/playoff/playoff-statistics/goalies.php';
}
elseif( get_segment('team') )
{
	include ROOT . '/pages/league/NHL/playoff/playoff-statistics/team.php';
}
elseif( get_get('other') == 'top' )
{
	include ROOT . '/pages/league/NHL/playoff/playoff-statistics/top.php';
}
else
{
?>
...
<?php
$count = $db->rows( $db->query("SELECT * FROM league_nhl_po_statistics WHERE pos = 'Player' AND league_id = '" . LEAGUE_ID . "'"));
...
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY gp ' . order_type('gp') . ' ' . $limit);
}
elseif(get_segment('sort') == 'rank')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY rank ' . order_type('rank') . ' ' . $limit);
}
elseif(get_segment('sort') == 'goals')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY goal ' . order_type('goals') . ' ' . $limit);
}
elseif(get_segment('sort') == 'assists')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY asst ' . order_type('assists') . ' ' . $limit);
}
elseif(get_segment('sort') == 'sa')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY goal / shot_sa ' . order_type('sa') . ' ' . $limit);
}
elseif(get_segment('sort') == 'plusminus')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY plus_minus ' . order_type('plusminus') . ' ' . $limit);
}
elseif(get_segment('sort') == 'shots')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY shot_sa ' . order_type('shots') . ' ' . $limit);
}
elseif(get_segment('sort') == 'hits')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY hits ' . order_type('hits') . ' ' . $limit);
}
elseif(get_segment('sort') == 'ppg')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY ppg_ga ' . order_type('ppg') . ' ' . $limit);
}
elseif(get_segment('sort') == 'shg')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY shg_eng ' . order_type('shg') . ' ' . $limit);
}
elseif(get_segment('sort') == 'pims')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY pims ' . order_type('pims') . ' ' . $limit);
}
elseif(get_segment('sort') == 'toi')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY time / gp ' . order_type('toi') . ' ' . $limit);
}
elseif(get_segment('sort') == 'points')
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY goal + asst ' . order_type('points') . ' ' . $limit);
}
else
{
	$res = $db->query('SELECT * FROM league_nhl_po_statistics WHERE pos = "Player" AND league_id = "' . LEAGUE_ID . '" ORDER BY goal + asst DESC ' . $limit);
}
$i = 0;
$num = 0;
...
echo '</table>';
if(!$count)
echo error("Līgā vēl nav izspēlēta neviena playoff spēle, tāpēc pašlaik nav statistikas ko ievākt!", "892px");
if($count)
echo $pager;
}
?>
...
if (isset($_GET['action']) && $_GET['action'] === 'push') {
	...
	$current = $db->get_row("SELECT * FROM gamescore WHERE game = 'vardes' AND user_id = '$auth->id'");
	$is_new_record = false;
...
if ($auth->ok) {
	...
	$user_high_score = (int) $db->get_var("SELECT score FROM gamescore WHERE game = 'vardes' AND user_id = '$auth->id'");
}
...
$today_scores = $db->get_results("SELECT * FROM gamescore WHERE game = 'vardes' AND time >= '$start_of_today' ORDER BY score DESC LIMIT 10");
...
$alltime_scores = $db->get_results("SELECT * FROM gamescore WHERE game = 'vardes' ORDER BY score DESC LIMIT 10");

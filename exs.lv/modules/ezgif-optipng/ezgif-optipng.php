...
if (isset($_POST['file'])) {
	...
	if (!empty($_POST['new-image-url']) && empty($_FILES['new-image']['name']) && substr($_POST['new-image-url'], 0, 1) != '.' && substr($_POST['new-image-url'], 0, 1) != '/') {
		...
		$filename = '/tmp/ezpng_' . uniqid() . '.png';
	...
	if ($foo->processed) {
		redirect('/optipng/' . $foo->file_dst_name);
	} else {
...
$tpl->assign('description', 'Optimize PNG images online to reduce file size');

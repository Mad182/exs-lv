...
var shiptypes = [["Mīnu meklētāju",2,4],["Fregati",3,4],[ "Kreiseri",4,2],[ "Līnijkuģi",5,1]];
...
function imagePreload() {
  ...
  for (i=0;i<ids.length;++i) {
    var img = new Image, name = "/modules/battleship/images/batt"+ids[i]+".gif";
    img.src = name;
    preloaded[i] = img;
  }
  window.status = "";
...
function setImage(y,x,id,ispc) {
  if ( ispc ) {
    computer[y][x][0] = id;
    document.images["pc"+y+"_"+x].src = "/modules/battleship/images/batt"+id+".gif";
  } else {
    player[y][x][0] = id;
    document.images["ply"+y+"_"+x].src = "/modules/battleship/images/batt"+id+".gif";
  }
}
...
function showGrid(ispc) {
  ...
  for (y=0;y<gridy;++y) {
    for (x=0;x<gridx;++x) {
      if ( ispc )
      document.write ('<a href="javascript:gridClick('+y+','+x+');"><img name="pc'+y+'_'+x+'" src="/modules/battleship/images/batt100.gif" width=16 height=16></a>');
      else
      document.write ('<a href="javascript:void(0);"><img name="ply'+y+'_'+x+'" src="/modules/battleship/images/batt'+player[y][x][0]+'.gif" width=16 height=16></a>');
    }
    document.write('<br>');
  }
}
...
function gridClick(y,x) {
  ...
  if (computer[y][x][0] < 100) {
    ...
    if ( --computersships[shipno][1] == 0 ) {
      ...
      alert("Tu nogremdēji manu "+shiptypes[computersships[shipno][0]][0]+"!");
      updateStatus();
      if ( --computerlives == 0 ) {
        alert("Tu uzvarēji!");
        playflag = false;
...
function computerMove() {
  ...
  if (player[sy][sx][0] < 100) {
    ...
    if ( --playersships[shipno][1] == 0 ) {
      ...
      alert("Es nogremdēju tavu "+shiptypes[playersships[shipno][0]][0]+"!");
      if ( --playerlives == 0 ) {
        knowYourEnemy();
        alert("Es uzvarēju");
        playflag = false;
...
document.write("<center><table id='battleyard'><tr><td align=center><p class='heading'>Pretinieka flote</p></td>"+
"<td align=center><p class='heading'>Tava flote</p></td></tr><tr><td>");
showGrid(true);

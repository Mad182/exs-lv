...
if (isset($_GET['action']) && $_GET['action'] == 'push') {
	...
	// Save High Score
	$prev_top = get_game_top_users('snake');
	$prev_best = (int) $db->get_var("SELECT MAX(score) FROM gamescore WHERE game = 'snake' AND user_id = '$auth->id'");
	...
	check_game_record_loss('snake', $auth->id, $prev_top);
	$highScore = max($prev_best, $score);

	if ($is_new_record) {
		push('Uzstādīja jaunu rekordu spēlē <a href="/snake">Čūska</a> (' . number_format($highScore, 0, '', ' ') . ' punkti)', '/bildes/icons/games/snake.png', 'game-snake-' . $auth->id);
	}
...
if ($active_sub === 'top') {
	...
	$scores = $db->get_results("SELECT user_id, MAX(score) as score, MAX(time) as time FROM gamescore WHERE game = 'snake' AND time >= '$start_of_today' GROUP BY user_id ORDER BY score DESC LIMIT 100");
	...
	$scores = $db->get_results("SELECT user_id, MAX(score) as score, MAX(time) as time FROM gamescore WHERE game = 'snake' GROUP BY user_id ORDER BY score DESC LIMIT 100");
	$tpl->newBlock('snake-top');
	...
	$user_high_score = 0;
	if ($auth->ok) {
		$user_high_score = (int) $db->get_var("SELECT MAX(score) FROM gamescore WHERE game = 'snake' AND user_id = '$auth->id'");
	}
	...
	$sidebar_scores = $db->get_results("SELECT user_id, MAX(score) as score FROM gamescore WHERE game = 'snake' GROUP BY user_id ORDER BY score DESC LIMIT 10");
	if ($sidebar_scores) {
		$i = 1;

...
function desas_overlay(text) {
  $('<div class="overlay">'+text+'</div>').hide().appendTo("#desas").fadeIn('slow').delay(3500).fadeOut('slow');
}
...
function load_desas(url) {
	...
	$.getJSON(url, function(data) {
				...
				$.each(val, function(line_key, line_val) {
					$.each(line_val, function(fkey, fval) {
					  if (fval == data.me) {
						...
						if (fval == 0) {
							$('<div class="field"><a href="/desas/?mark='+line_key+fkey+'">#</a></div>').appendTo("#desas");
						}
		...
		if(data.other == 0) {
			check_opponent('Meklē pretinieku');
		}
		if(data.timeout == 1) {
			timeout = true;
		}
	...
	if(timeout != true) {
		desasRefreshId = setInterval("load_desas('http://exs.lv/desas?xmlhttprequest=true')",2000);
	} else {
		desas_timeout();
	}
}

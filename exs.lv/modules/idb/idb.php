...
if (isset($_GET['var1']) && (in_array($_GET['var1'],array('jqnew','jqupd','jqapp'))) && isset($_GET['var2'])) {
	...
	if ($get_items) {
		...
		for ($i=1; $i<6; $i++) {
			...
			} else $ret .= '<a class="idb-next" href="/db/'.$type.'/'.$i.'">'.$i.'</a>&nbsp;';
		}
		$ret .= '</li>';
		echo $ret;
	} else {
	...
	if (isset($_GET['var2'])) {
		...
		// priekšmeta formas apskate
			...
			if ($item) {
				...
				} else $tpl->newBlock('myitem-submit-disabled');
	...
	foreach ($main_blocks as $block) {
		...
		if ($item_list) {
			...
			for ($a=0; $a<5; $a++) {
				...
				} else $concat .= '<a class="idb-next" href="/db/'.$block[3].'/'.($a+1).'" title="'.($a+1).'. lapa">'.($a+1).'</a>&nbsp;';
			}

...
/**
 * Tetris spēle ar pret-krāpšanās (anti-cheat) pārbaudēm un rezultātu saglabāšanu
 */

// 1. AJAX game session token creation
if (isset($_GET['action']) && $_GET['action'] == 'init_token') {
	header('Content-Type: application/json');
	$token = bin2hex(random_bytes(16));
	$_SESSION['tetris_token'] = $token;
	$_SESSION['tetris_start_time'] = time();
...
if (isset($_GET['action']) && $_GET['action'] == 'push') {
	...
	// Anti-Cheat Check 1: Session Token Validation & One-Time Use
	if (empty($_SESSION['tetris_token']) || empty($token) || !hash_equals($_SESSION['tetris_token'], $token)) {
		echo json_encode(['success' => false, 'message' => 'Nekorekts spēles žetons (Token verification failed).']);
		exit;
	}

	$start_time = isset($_SESSION['tetris_start_time']) ? intval($_SESSION['tetris_start_time']) : time();
	$duration = max(1, time() - $start_time);

	// Clear token to prevent replay attacks
	unset($_SESSION['tetris_token']);
	unset($_SESSION['tetris_start_time']);
	...
	// Anti-Cheat Check 2: Minimum Duration Threshold
	// Real Tetris game yielding > 10,000 points takes at least 15-20 seconds
	if ($score > 5000 && $duration < 8) {
		echo json_encode(['success' => false, 'message' => 'Spēles ilgums ir par īsu uzrādītajam rezultātam!']);
		exit;
	}

	// Anti-Cheat Check 3: Maximum Theoretical Score Limit per Line & Level
	// In Tetris, max base score for 4-line Tetris is 800 * level. Adding hard drop bonuses (~40/piece),
	// max reasonable score per line cleared is around 1200 * max_level + 2000 base.
	$max_level = max(1, $level);
	$max_possible_score = ($lines * 1400 * $max_level) + 3000;

	if ($score > $max_possible_score) {
		echo json_encode(['success' => false, 'message' => 'Uzrādītais rezultāts pārsniedz maksimālo pieļaujamo robežu!']);
		exit;
	}

	// Anti-Cheat Check 4: Rate of Score Gain per Second
	// Even a speed-runner cannot exceed ~2000 points per second sustained
	if (($score / $duration) > 2500 && $score > 10000) {
	...
	if ($is_new_record) {
		push('Uzstādīja jaunu rekordu spēlē <a href="/tetris">Tetris</a> (' . number_format($highScore, 0, '', ' ') . ' punkti)', '/bildes/icons/award_star_gold_3.png', 'game-tetris-' . $auth->id);
	}

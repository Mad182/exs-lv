...
bkcore.hexgl.HexGL.prototype.displayScore = function(f, l)
{
	...
	if(this.gameover !== null)
	{
		...
		this.gameover.children[1].innerHTML = tf.m + "'" + tf.s + "''" + tf.ms;
		if (this.containers.main && this.containers.main.parentElement) {
			this.containers.main.parentElement.style.display = "none";
		}

		if (this.gameplay.result == this.gameplay.results.FINISH) {
			var score = Math.max(500, Math.round(300000 - f));
			if (window.hexGLSessionToken) {
				var formData = new FormData();
				formData.append('token', window.hexGLSessionToken);
				formData.append('score', score);
				formData.append('duration', Math.round(f / 1000));
				...
				.then(function(data) {
					var msgEl = document.getElementById('score-push-msg');
					if (msgEl) {
						if (data.success) {
							if (data.isNewRecord) {
								msgEl.innerHTML = '🎉 Jauns personīgais rekords: ' + Number(score).toLocaleString() + ' punkti!';
							} else {
								msgEl.innerHTML = 'Rezultāts saglabāts topos (' + Number(score).toLocaleString() + ' pnk)';
							}
						} else {
							msgEl.innerHTML = data.message || 'Rezultāts netika saglabāts.';
						}
					}
				})
				.catch(function(err) { console.log('Score push error:', err); });
			}
		} else {
			var msgEl = document.getElementById('score-push-msg');
			if (msgEl) msgEl.innerHTML = 'Kuģis tika iznīcināts!';
		}
		return;
	}

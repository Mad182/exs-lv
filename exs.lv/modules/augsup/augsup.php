...
if (isset($_GET['action']) && $_GET['action'] === 'push') {
	...
	// Save High Score
	$prev_top = get_game_top_users('augsup');
	$prev_best = (int) $db->get_var("SELECT MAX(score) FROM gamescore WHERE game = 'augsup' AND user_id = '$auth->id'");
	...
	check_game_record_loss('augsup', $auth->id, $prev_top);
...
if ($auth->ok) {
	...
	$user_high_score = (int) $db->get_var("SELECT MAX(score) FROM gamescore WHERE game = 'augsup' AND user_id = '$auth->id'");
}
...
$today_scores = $db->get_results("SELECT user_id, MAX(score) as score FROM gamescore WHERE game = 'augsup' AND time >= '$start_of_today' GROUP BY user_id ORDER BY score DESC LIMIT 10");
...
$alltime_scores = $db->get_results("SELECT user_id, MAX(score) as score FROM gamescore WHERE game = 'augsup' GROUP BY user_id ORDER BY score DESC LIMIT 10");
